...
export async function POST(req: NextRequest) {
  ...
  const { display_name, display_emoji } = await req.json();
  if (!display_name) return NextResponse.json({ error: "Nome obrigatório" }, { status: 400 });
  ...
  const ctx = { ...((prefs?.context as Record<string, unknown>) || {}), community_name: display_name, community_emoji: display_emoji || null };
